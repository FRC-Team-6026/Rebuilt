...
public class KrakenController {
    ...
    public KrakenController(int canbusNumber, KrakenInfo Info) {
        ...
        CANSparkMaxUtil.Usage usage = Info.canbusUse;
        if (usage == Usage.kAll) {
            motor.getVelocity().setUpdateFrequency(100); // 100Hz = 10ms between frames
            motor.getPosition().setUpdateFrequency(100);
        } else if (usage == Usage.kPositionOnly) {
            motor.getVelocity().setUpdateFrequency(10);  // 10Hz = 100ms between frames
            motor.getPosition().setUpdateFrequency(100);
            ...
            motor.getVelocity().setUpdateFrequency(5);
            motor.getPosition().setUpdateFrequency(5);
        }
    }
}